...
これら3つのドキュメントは「あるべき姿」ではなく「今のアプリの実際の動き」を記録することを目的としている。ドキュメントを更新しないままコードだけ変更すると、次にこのリポジトリを触るとき(人間・Codex問わず)に誤った前提で作業することになるため、コード変更とドキュメント更新は同じコミット/同じタスクの中で完結させること。挙動に曖昧な点(未実装機能、エラー時の非対称な挙動など)がある場合も、推測で「あるべき姿」を書かず、実際に動かして確認した挙動を記載する。

## 必須ルール: 同じ性質のコードは同じ構成に統一する

- 同じ性質・責務を持つコードは、特殊な分岐が必要な場合を除き、同じディレクトリ構成・責務分担・命名・エラー処理で実装する。
- 新しい構成を導入した場合は、新規コードだけへ適用して終わらせず、同じ性質を持つ既存コードも同じタスク内で新構成へ移行する。
- 一時的な互換実装や旧構成の使い回しによって、新旧二種類の実装方式を残さない。段階移行が不可避な場合は、対象範囲・理由・解消条件をコードとドキュメントへ明記する。
- API呼び出しは`frontend/src/api/apiClient.ts`へ通信・認証・共通エラー処理を集約し、`frontend/src/api/domains/`でドメイン別のパス・入出力型・個別のフォールバックメッセージを定義する。Reactからの利用は対応する`frontend/src/api/hook/`を経由する。
